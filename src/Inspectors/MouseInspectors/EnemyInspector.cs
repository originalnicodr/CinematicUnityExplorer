...
{
    public class EnemyInspector : MouseInspectorBase
    {
        ...
        {
            yield return null;
            EnemyInspectorResultsPanel panel = UIManager.GetPanel<EnemyInspectorResultsPanel>(UIManager.Panels.EnemyInspectorResults);
        ...
        public override void UpdateMouseInspect(Vector2 mousePos)
        {
            ...
            var healthManagerType = ReflectionUtility.GetTypeByName("HealthManager");

            currentHitObjects.AddRange(
                hits
                    .Where(hit => hit.transform != null)
                    .Where(hit => hit.transform.GetComponent(healthManagerType) != null)
                    .Select(hit => hit.transform.gameObject)
                    .Distinct() // 避免重複加入同一個物件
            );
